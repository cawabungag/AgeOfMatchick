...
namespace Common
{
	public class GameUiCanvas : MonoBehaviour, IGameUiCanvas
	{
		...
		[SerializeField]
		private InteractableDropdown _iconsSetDropdown;
		[SerializeField]
		private InteractableDropdown _fillStrategyDropdown;
		[SerializeField]
		private InteractableButton _startGameButton;
		...
		private void Start()
		{
			_iconsSetDropdown.AddItems(_appContext.Resolve<IconsSetModel[]>()
				.Select(iconsSet => iconsSet.Name));
			_fillStrategyDropdown.AddItems(_appContext
				.Resolve<IBoardFillStrategy<IUnityGridSlot>[]>()
